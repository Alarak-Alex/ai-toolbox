...
/// Command timeout, matching the Pi CLI's own `resolveConfigValue` limit.
const COMMAND_TIMEOUT: Duration = Duration::from_secs(10);

/// Environment a Pi config value is resolved against.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ConfigValueHost {
    /// Pi runs against the host machine, so `$VAR` / `!cmd` use the host env.
    Local,
    /// Pi runs inside WSL Direct, so values must be resolved in that distro.
    Wsl { distro: String },
}
...
fn wsl_command(distro: &str, program: &str, args: &[&str]) -> Command {
...
/// "set to an empty string" distinguishable the way the Pi CLI treats them.
async fn lookup_env_var(name: &str, host: &ConfigValueHost) -> Option<String> {
