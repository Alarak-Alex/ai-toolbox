...
- 主模型的默认思考档位和 `model` 走同一个指针：只要本次操作让目录行成为/继续作为主模型，就把该行的 `defaultReasoningLevel` 投影进 `config.toml` 的顶层 `model_reasoning_effort`（由 `resolveCodexDefaultReasoningEffort` + `buildCodexSettingsConfig({ reasoningEffort })` 完成）。该行没有声明默认档位时必须保持 `model_reasoning_effort` 原值不动，不能用空值清掉。编辑主模型行的默认档位、删除/批量删除/连通性移除导致默认模型回落到新行、以及首次采用目录第一项时都按同一规则处理；与主模型无关的目录编辑不得改写它。
