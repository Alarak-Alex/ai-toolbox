...
- `SessionManagerPanel` 的 Collapse 标题和内容区搜索栏都要 sticky：标题钉在内容区 `top: 0`（`main` 已用 `--content-top-offset` 垫过），搜索栏钉在标题下方。冻结只属于当前会话卡片，卡片滚出视口后必须一起解冻；这依赖全局 `.ant-collapse` 的 `overflow: visible`，不要在模块样式里把卡片重新设成 `overflow: hidden`。
