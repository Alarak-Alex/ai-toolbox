...
- Gateway 页顶栏（状态/启停/内部 Tab）和统计/明细筛选栏都要 sticky：顶栏钉在内容区 `top: 0`（`main` 已用 `--content-top-offset` 垫过），筛选栏钉在顶栏下方。滚动浏览表格时仍可操作；页面滚出后自动解冻。不要改成 `fixed`，也不要把 offset 再写进 sticky `top`。
