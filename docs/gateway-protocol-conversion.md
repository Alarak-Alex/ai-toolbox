...
| StepFun | 参考项目按平台/模型启用：`2603` 支持 low/high effort，`step-3.7-flash` 支持 low/medium/high，无后缀 `step-3.5-flash` 不发 effort。 | 没有单独 `ProviderBodyCompat`；通过 `codexChatReasoning` 矩阵覆盖。`thinkingParam=none`、`effortParam=reasoning_effort`；provider 已识别为 `stepfun` 后按模型分档：`2603` 用 `effortValueMode=low_high`（medium 收敛为 high），`step-3.7-flash` 用 `passthrough`（low/medium/high 原样下发），其余含无后缀 `step-3.5-flash` 不发 effort。 | 内置 StepFun 当前在 Codex/Gemini profile 中出现；模型名只作为已识别 provider 内的能力细分，不作为 custom provider 的供应商识别来源。参考项目另有“模型名含 `step-3.5-flash-2603` 即套规则”的聚合渠道分支，本项目明确不吸收。 |
...
关闭连接由 runtime 完成有界 Close 握手：上下游并行收尾，各自最多 1 秒；发送或冲刷自动 Close 回复后继续读取，直到握手结束、传输错误或超时。期间到达的帧只排空，不再转发、建立新轮次或改变已结算用量。`tokio-tungstenite` 的发送 Close 不等于握手完成，收到 Close 后也必须冲刷自动回复；过早释放 TCP 在 Windows 上可能使客户端收到 `10054 ConnectionReset`。此处使用 `SinkExt::close` 兼容已经收到/发送 Close 的状态，然后驱动读侧结束，不改协议转换或 provider wire 适配。

本地网络回归位于 `runtime/websocket/tests.rs`、`runtime/websocket/lifecycle_tests.rs` 和 `runtime/websocket/settings_tests.rs`，覆盖真实升级、双轮复用、多路交错/重复终态、默认关闭与保存后即时生效、关闭时在途用量保留、空闲读/握手期间关闭、转换前 426 与后续 HTTP 转换、认证/限流、握手重试预算、代理/RawURL/query/Beta header、预热、正文关闭/截断、写入失败、缺终态、超时和停止。关闭握手回归还覆盖 Close 已发出后才到达的真实 TCP 请求、等待对端确认、任一端主动关闭时冲刷回复，以及对端不回应时有界退出；就绪同步通过 Ping/Pong 和 TCP peek，不依赖固定 sleep。`settings.rs` 验证旧配置默认关闭、开关保存读回及相邻设置保留。跨层回归还包括 `session_import/websocket_tests.rs` 的双向去重、`usage_stats.rs` 的日聚合、`commands.rs` 的导出、`cli_proxy/mod.rs` 的接管/恢复及 `tauri/tests/sqlite_jsonb.rs` 的 v20→v21 升级。
...
|---|---|---|---|---|
| cc-switch | `../cc-switch` | `origin/main` | `970a6e4fbc2f4f93e62d52634c1924651a23a406` | `ebbf141fc71547a99f669df1be8e345130d1d890..970a6e4fbc2f4f93e62d52634c1924651a23a406` |
| AxonHub | `../axonhub` | `origin/unstable` | `5653bf0ae7a60c5febb6bbaec94eb2442103f8cb` | `dfbe22593ea33d62d1bc04d47a8e5d6c8b25d2bf..5653bf0ae7a60c5febb6bbaec94eb2442103f8cb` |

首次增量审阅范围（cc-switch 初始对齐）为 `a377d79303bc1e592d2783d559ca5bd6b8ba1417..878c26f31e012ba32b9772bd080bd4fa9e7d495e`；其后 2026-08-02 推进到 `ebbf141f`，2026-09-21 推进到 `970a6e4f`。表中“本次增量审阅范围”列始终记录最新一次推进，历史区间见对应日志条目。
...
- **2026-09-03 将 AxonHub baseline 从 `7d095b63` 推进到 `dfbe2259`**，审阅增量 `7d095b63..dfbe2259`（145 个提交，其中后端 `llm/`+`internal/` 约 45 个；`889bc8ee` 已在此前的流式终态判定工作中单独吸收）。以下按当前源码更新吸收状态：
  ...
  - **候选-低优先**：`d1cde099` Responses type-only `tool_choice`（`{"type":"image_generation"}` 无 name）——AI Toolbox Responses 方向已有 `RESPONSES_RAW_TOOL_CHOICE_METADATA_KEY` raw 保留兜底，其 `allowed_tools` 子集已在 2026-09-22 一轮吸收（见下方 `d5237439` 条目），剩余的有损点是「无 name 的普通 named choice 转 Chat/Anthropic target 时被丢弃」，由 `tool_choice_to_openai_and_anthropic_drop_type_only_choice` 锁定；`dfbe2259` 中 GLM/Zai `user_id` 需钳制 6-128 字符（Claude Code `metadata.user_id` 约 150 字符 JSON 触发 error 1214）——影响 Anthropic 直通 GLM anthropic 端点场景，直通字节保真语义下不便处理，等用户报告；`833853eb` Gemini fileData/image URL MIME 推断——我们响应式图片替换依赖显式 `image/*` mimeType，有反应式重试兜底。
...
2026-09-16 issue #356 定点修复：instruction（`system`/`developer`）消息位置改为按来源方向门控。参考快照为 cc-switch `e098279934a6041ebab35664c5fbd785df055ee0`（本地 checkout 与当时 `origin/main` 的提交；§19.4 表当时记录的 cc-switch baseline 是 `ebbf141f`，`e0982799` 是其之后的提交。2026-09-15 条目中的 `42ac174d` 只是该次专项核对用的参考快照，从未成为 baseline，原表述有误，已在此更正）与 AxonHub `dfbe22593ea33d62d1bc04d47a8e5d6c8b25d2bf`（本地 checkout == §19.4 记录的 `origin/unstable` baseline）。两条关键参考提交（cc-switch `b724f5dd`、AxonHub `9dfd6ac0`）同时存在于本次快照与各自 baseline 之内，因此本次不推进 baseline，也不改写参考仓库工作树。
...
**2026-09-21 将 cc-switch baseline 从 `ebbf141f` 推进到 `970a6e4f`**（`origin/main`，2026-09-21 tip）。增量审阅 `ebbf141f..970a6e4f` 共 227 个提交，其中触及 `src-tauri/src/proxy/**` 的 50 个、触及 `src-tauri/src/**` 的 125 个；2026-09-15（issue #352）与 2026-09-16（issue #356）两次都只是定点核对，本轮补齐中段 `e0982799..42ac174d` 与后续 `42ac174d..970a6e4f` 的逐条分类。吸收三条：

- **已经吸收：Anthropic -> Responses 的 `max_output_tokens` 下限钳制**（参考 cc-switch `6e4b0e6e`，其 #7287 / #7103）。Responses API 拒绝 `max_output_tokens < 16`，而 Claude Desktop 的模型可用性探测发 `max_tokens=1`，原样转换会让严格上游返回 400、客户端把已映射模型判定为不可用。实现位于 Responses target writer `transformer/openai/responses/request.rs::llm_request_to_responses`（常量 `RESPONSES_MIN_MAX_OUTPUT_TOKENS = 16`）：`1..=15` 抬到 16，`16+`、`0` 与其它值保持原有透传语义。选择 transformer 而不是 runtime middleware，是因为这是 Responses 目标协议的 wire 约束，与 provider meta / `defaultMaxTokens` 无关，缺少 provider meta 时也必须生效。回归：`responses_writer_clamps_sub_floor_max_output_tokens`（writer 单元，含 `max_completion_tokens` 别名与 `0` 透传）、`anthropic_probe_max_tokens_is_clamped_to_responses_floor`（Anthropic 来源 -> Responses 目标端到端）。
- **已经吸收：DeepSeek vendor mirror 的 `supports_search_tool`**（参考 cc-switch `5a040348`，其 #6653 / #6647）。DeepSeek Responses API 没有 `tool_search`，而 Codex 用 `search_tool_enabled = supports_search_tool && namespace_tools` 决定是否把 MCP 工具藏在 `tool_search` 后面；native `/responses` 直通路径上网关无法代它实现 tool_search，工具因此永远调不到。内置官方镜像 `tauri/resources/codex_deepseek_catalog_template.json` 的 `deepseek-flash` 改为 `false`（`deepseek-v4-pro` 原本已是 `false`），与 cc-switch `origin/main` 的同名模板一致。**neutral 模板保持 `supports_search_tool: true` 不变**：跨协议（Chat/Anthropic）目标由网关把 `tool_search`/namespace 展平后下发，Codex 侧保留官方 defer 语义是既有设计；测试 `non_deepseek_or_non_native_provider_keeps_neutral_template` 现在锁死这条差异，vendor 镜像侧由 `deepseek_host_native_catalog_mirrors_official_entries` 断言 `false`。
- **已经吸收：StepFun 的 per-model effort 分档**（参考 cc-switch `3f75bbdf`）。厂商 reasoning 指南与两站模型页：`step-3.5-flash-2603` 支持 low/high，`step-3.7-flash` 支持 low/medium/high（默认 medium），无后缀 `step-3.5-flash` 不暴露 effort。`runtime/upstream.rs::infer_codex_chat_reasoning_config` 的 StepFun 分支改为 `supports_effort = model.contains("2603") || model.contains("step-3.7-flash")`，`effort_value_mode` 按模型分叉：`2603` 保持 `low_high` 收敛映射，`3.7-flash` 用 `passthrough`（套 `low_high` 会把 medium 塌成 high，造出 wire 上无差异的假档位）；`thinking_param` 仍为 `none`。规则留在 inference table 而不是 preset 级 `codexChatReasoning`，因为后者是 provider 级、会丢掉 per-model 门控。回归：`provider_compat_stepfun_uses_per_model_effort_tiers`（5 组模型 x 档位）。

- **明确不吸收：StepFun 的模型名识别分支**。cc-switch 该块还有 `haystack.contains("step-3.5-flash-2603")` 分支，用于“经聚合渠道跑该模型、但平台名/base_url 不含 stepfun”的场景。本项目 §5.19 与 §6 明确禁止仅凭 body `model` 识别供应商（避免 custom provider 被误套供应商方言），只保留“已识别 `providerType=stepfun` 之后的模型内能力细分”。
- **待确认：StepFun 的 catalog 档位显示**。cc-switch 同一提交还给其 Codex 预设声明 `reasoningLevels`（3.7-flash low/medium/high、2603 low/high）。本项目 Codex catalog 的 `supported_reasoning_levels` 由 `preset_models.json` 的 `variants` 推导，而该文件同时服务 OpenCode/Pi/OMP，补 variants 会改变这些工具的模型元数据，属跨工具产品决策，本轮不动。现状态：StepFun 预设声明 `reasoning: true` 且无 variants，Codex 目录推导出 `low/high/max`；由于 3.7-flash 现在走 `passthrough`，用户选择 `max` 会原样下发到上游（此前会被收敛成 high）。需要单独决定 Codex-scoped 的 per-model 档位来源。
- **待处理（真实缺口，需单独一轮）**：`b7da894b` / `914c8bb5` / `0a9a4378` / `bf325b25` / `dfc9b066` / `9e110053` / `054673e0` / `d05a11cc` 的 xAI native Responses sanitize 系列（根级 `oneOf/anyOf` 折叠、完成态 function_call 参数整型浮点改写与 2^64 拒绝、Codex multi-agent v2 `agent_message` input item 改写、未知 subagent SKU 回落到配置模型）。本项目 `runtime/compat/xai_responses.rs` 对齐的是更早的 `dbb5bd15`，目前只有 namespace flatten/restore、additional_tools 提升、tool 去重、null `reasoning_content` 清理、unsupported tool 过滤等子集，上述四类行为没有对应实现；属同一条 xAI native Responses 路径的真实缺口。
- **待处理（真实缺口，需单独一轮）**：`46f19a15` DeepSeek Chat 的 `prompt_cache_hit_tokens` / `prompt_cache_miss_tokens` 兜底——本仓 `usage_parser.rs` 只读 `input_tokens_details` / `prompt_tokens_details` 的 `cached_tokens`（另有 Kimi 顶层 `cached_tokens` 特例），DeepSeek 文档化字段缺失时会记 0 缓存命中、虚高成本；`12296aeb` 多字节 UTF-8 截断 panic——本仓 gateway 无 `mask_url`，但 `open_code/commands.rs` 与 `open_claw/commands.rs` 的解析失败预览仍是 `&content[..500]` 按字节切，含中文的配置会 panic；`9db9c56f` “工具调用全被丢弃时不得伪造成完成”——本仓 Chat -> Responses 对缺 name 的工具调用没有对应失败终态，需先核对 F-3 门控后的实际行为。
- **待确认（厂商语义）**：`c8262476` Kimi/Moonshot 撤出 reasoning 注入。厂商 2026-08 反馈其端点不再需要 tool-call reasoning 回放，注入占位符反而扰乱思维链。本仓 `backfill_tool_call_reasoning_content` 与 Anthropic tool thinking 规范化仍对 `providerType=moonshot|kimi` 生效，且 `docs/gateway-provider-compatibility.md` §5.4 把它记为有意设计；改前需要厂商/回归证据。
- **已核对，结论为已覆盖或不适用**：`b78192e8` 空 `reasoning_content` 占位符（本仓 `transformer/stream.rs` reasoning 分支已有 `is_empty` 门控）、`7726c834` DeepSeek vision catalog（已由本仓 `29ef8420` / issue #368 覆盖）、`872ec775` Codex OAuth parallel tool calls（`ProviderBodyCompat::CodexOfficial` 已强制 `parallel_tool_calls=true`，见 §14）、`d8065cc6` 中途 system 保留（已由 F-15 方向门控覆盖）、`6b8f3643` 的 proxy body 上限（本仓 runtime 已有 16 MiB 请求体/解压上限；其 usage script 沙箱与 Grok 会话日志上限在本仓无对应模块）、`38cfafdc` Claude takeover 模型别名（本仓 Claude 接管不改写客户端模型别名）、`db346128` / `6243e20a` Codex OAuth 客户端身份（本仓透传客户端 `originator`/`version`，不存在用自家版本号伪造 Codex 身份的路由）、`84e75ad2` Pi CLI 接管（本仓 `GatewayCliKey` 不含 Pi）、`e724270d` / `17be9092` / `e0982799` 图片端点代理（本仓 gateway 不代理 images 端点，Image 能力走独立模块）、`bdeaac75` Alpha Search 与 Claude hosted WebSearch（新端点 + 新桥接特性，非缺陷修复；本仓按 raw fragment/lossy 处理 Responses hosted tool 声明）、`8272707d` / `33c80626` skills 打包安装（本仓 skills 走 git clone，无 zip 归档路径）、`fdbe3a85` OpenCode 模型批量添加与 `a659440b` 提示词外部编辑刷新（前端交互特性，无 gateway 语义）、`06082e18` MiniMax Code harness、`926af949` 编辑即投影 live 配置、`93bb91aa` 配置迁移路由、账号体系类（`a2e22f33`、`92d52916`、`c2ec78dd`、`0455a92c`、`f62c854a`、`15884b20`）、预设/定价/catalog 展示与 CI/i18n 类（`48e572cc`、`e12fc623`、`1435223b`、`746e2288`、`40cac1a6`）：本仓无对应模块、属产品/前端层，或语义已被既有实现覆盖，本轮维持未吸收。

- 最终验证：`cargo test --jobs 2` 全绿（lib 2383 passed / 10 ignored，coding 集成 185、deeplink 4、sqlite_jsonb 27、doctest 10，全部 0 failed；含仓库自带 compat registry 守卫 `compat_rule_registry_has_unique_names_and_static_evidence`，它在本轮首次抓到 StepFun 测试改名后注册表漏改）。`pnpm exec tsc --noEmit` 通过。`pnpm test` 713/714 通过，唯一失败 `web/test/features/coding/shared/sessionManager/sessionManagerDetailDomain.test.ts` 是本机 pnpm store 的 `node_modules/.pnpm/lucide-react@0.563.0_react@19.2.3/node_modules/react` 为不可信装入点（Cannot read package config ... unknown error），与本轮改动无关。改动文件的 rustfmt 检查：本轮新增代码无差异；`codex/commands.rs` 另有 8 处改动前就存在的偏差，已用 HEAD 版本复核确认为既有问题。
- **2026-09-22 将 AxonHub baseline 从 `dfbe2259` 推进到 `5653bf0a`**（`origin/unstable`，2026-09-22 tip；本地 checkout 工作树干净、`HEAD == origin/unstable`，未改写参考仓库）。增量审阅 `dfbe2259..5653bf0a` 共 95 个提交，其中触及 `llm/`、`internal/`、`integration_test/` 的 72 个；其余集中在 channel、quota、OIDC/信任代理、webhook、trace、分页与前端。吸收三条：

  - **已经吸收：Anthropic 目标保留客户端 cache_control 断点**（参考 AxonHub `7444f537`，其 #2342）。参考结论是不要把客户端已规划的 prompt-cache 断点重新规划：Anthropic 只允许 4 个 `cache_control` 断点，而本仓 `runtime/cache_injector.rs` 的 message anchor 始终按“距末尾 20 块窗口”选择，位置每轮都变，会让序列化前缀 hash 改变、每轮都付满额 cache write。实现现在按 `normalize_message_contents` -> `sanitize_unsupported_cache_controls` -> `trim_cache_controls_to_limit(body, 4)` 顺序整理，随后只在有剩余预算时补结构锚点（缺什么补什么，不再无条件重建 tools/system 断点；tools 先于 system 且**消耗预算、总数绝不越过 4**，否则 Anthropic 会直接 400），并且仅当 `count_message_breakpoints(body) == 0` 时才注入 planner 的 message anchor。客户端断点超过 4 个时确定性裁剪：先丢最早的 message 断点，再丢最早的 tools/system 断点。回归：`cache_injector` 的 `preserves_client_structural_breakpoints_and_fills_message_anchors`、`keeps_client_message_breakpoints_without_replanning`（断言 body 字节完全不变）、`trims_excess_client_breakpoints_deterministically`、`keeps_structural_anchors_within_breakpoint_limit`（3 个客户端断点 + 未打点 tools/system 时只补 tools，总数保持 4）、`keeps_string_system_untouched_when_budget_is_spent`。

  - **已经吸收：Codex official 出站剥离 `user`**（参考 AxonHub `147e6791`，其 #2352）。Chat 侧客户端仍可能带顶层 `user`，而 ChatGPT Codex 后端对该字段直接返回 400。实现位于 `runtime/upstream.rs::apply_codex_official_responses_body_compat`，与既有的 `max_tokens`/`max_completion_tokens`/`metadata` 清理一致。回归：`provider_body_compat_codex_official_responses_forces_required_fields`。

  - **已经吸收：`allowed_tools` tool_choice 保真**（参考 AxonHub `d5237439`，其 #2519 / #2504）。参考语义是 `mode` 与工具子集必须整体保留，只保留 `mode` 会静默放开调用方的白名单、把子集塞进空 function name 也会写出非法形态。AI Toolbox 原先在 `tool_choice_from_openai` 里对 Responses 扁平形态只取 `mode` 并降级成字符串（`{"type":"allowed_tools","mode":"auto","tools":[...]}` -> `"auto"`），Chat 嵌套形态则变成空 name 的 `Named` 后在各 writer 被丢弃。本轮在 IR 增加 `ToolChoice::AllowedTools(AllowedTools { mode, tools })`：入站同时识别 Chat 嵌套（`allowed_tools.mode/tools`）与 Responses 扁平（顶层 `mode`/`tools`）两种形态，selector 保持原 wire 对象；出站时 Chat writer 写回嵌套形态并把 function selector 归一为 `{type, function:{name}}`，Responses writer 写回扁平形态与 `{type, name}`，非 function selector（例如 MCP `{"type":"mcp","server_label":"local"}`）没有形态差异，原样转发。Anthropic/Gemini 目标没有白名单形态：Anthropic 只保留 mode（`required`/`any` -> `{"type":"any"}`，无 mode 时丢弃），Gemini 只保留 mode 对应的 `functionCallingConfig.mode`（无 mode 时按既有 type-only 行为输出 `ANY` 且不锁定名字）。Responses 目标的 raw sidecar（`RESPONSES_RAW_TOOL_CHOICE_METADATA_KEY`）保持不变，因此 Responses -> Responses/compact 仍是字节级保真。回归：`shared/messages.rs` 的 `tool_choice_from_openai_preserves_allowed_tools_subsets`、`tool_choice_allowed_tools_rebuilds_each_wire_shape`，以及 kernel 级 `allowed_tools_tool_choice_keeps_subset_across_openai_routes`（Responses -> Chat -> Responses 往返 + Anthropic/Gemini 降级）。

  - **不吸收 AxonHub 的 raw-vs-structured 一致性守卫**：其 `rawToolChoiceMatchesCurrentTools`（`responses/request_extensions.go`）在 raw sidecar 与当前结构化 tool_choice 不一致时放弃 raw、改用结构化结果。本仓 IR 在一次请求内不会被改写 `tool_choice`，且 raw sidecar 只在“同一次 Responses 入站 -> Responses 兜底出站/compact”路径恢复，不存在参考项目那种序列化/再解析后失配的场景；直接照搬只会增加一个本仓不可达的比较分支。

- **2026-09-22 AxonHub 增量中的待处理与明确不吸收项**（同一区间 `dfbe2259..5653bf0a`）：

  - **待处理（需要跨层方案，本轮已回退）**：`834eea2b` GLM-5.2+/5.3 方言（GLM-5.3 恒开思考）。本仓 Z.ai 的 `thinking.type` 有两个写入点：`runtime/upstream.rs::apply_zai_openai_chat_thinking_compat`（body compat 层）与 Codex -> Chat reasoning 矩阵 `apply_codex_chat_reasoning_config`（有 provider meta 的目标路径会在 `reasoning_enabled == false` 时清 `reasoning_effort`、写 `reasoning.effort`/`thinking_param`）。只改前者会让“有 provider meta”与“无 provider meta”两条路径分叉，并可能被后者覆盖，因此本轮完整回退，不改半成品。要做必须在 reasoning 矩阵里统一决定 `thinking.type` 的最终值，并补 5.2+/5.3 与 GLM-4.x 的分档回归。
  - **待确认（架构/产品决策）**：`19a3c27d` 把 Anthropic `max_tokens` 默认值改为模型卡输出上限。本仓 `transformer/anthropic/outbound.rs` 仍固定 `8192`，transformer 按架构不能读模型目录；若要吸收，需要由 runtime 基于 provider meta/模型卡注入默认值（与既有 `defaultMaxTokens` 上限补齐/截断同一位置），属新增设计而不是补字段。
  - **明确不吸收**：`fd4158ef` 多条 system 合并 + “迟到 system 提到最前”与本文 §19.4 记录的 F-15 / issue #356 结论（Anthropic 来源保序、迟到 instruction 原位降级为 `user`）相反，继续不采纳；`35434951` 请求体上限本仓已有 16 MiB（`runtime/http_io.rs` `MAX_REQUEST_BODY_BYTES`、`content_encoding.rs` `MAX_DECOMPRESSED_BODY_BYTES`）；`cb62c2a1` 已被 `67e51377` revert，净效果为零；`9ba9a5c7`、`d46fbcf4` 属 images 端点，本仓不代理；`f4fffa76`、`2cf12695` 等 OIDC/信任代理/账号/安全类，以及 channel、quota、webhook、trace、分页、前端类提交，本机 CLI 网关没有对应物。

- **最终验证（2026-09-22 AxonHub 一轮）**：`cargo check --lib --no-default-features` 通过（该命令在本轮抓到一处只有 `cargo test` 看不出的缺陷：`cache_injector.rs::count_cache_controls` 被新生产代码调用却仍带 `#[cfg(test)]`，已去掉属性）；`cargo test --jobs 2 --lib --no-default-features transformer::` 294 passed；`cargo test --jobs 2 --lib --no-default-features`（cache_injector、upstream body compat、allowed_tools、tool_choice 相关 24 项）全绿；随后按根规则执行的全量测试结果见提交说明。
